...
%% defaults
if ~isfield(p,'type'),      p.type = 'mfcc';         end
if ~isfield(p,'fs'),        error('sampling frequency (p.fs) not specified'); end
if ~isfield(p,'mel_filt_bank'),  p.mel_filt_bank = [20 16000 40]; end %40 mel frequency bands
...
c.fft_freq = linspace(0,p.fs/2,p.fft_size/2+1); %p.fft_size/2+1 = 129 Hz frequency bins
...
if strcmp(p.mel_filt_bank,'auditory-toolbox'),
    ...
    c.num_filt = p.mel_filt_bank(3); %p.mel_filt_bank(3) = 40 mel frequency bands
    ...
    for i=1:c.num_filt+2,
        [tmp.dummy tmp.f_idx(i)] = min(abs(tmp.mel - tmp.m_idx(i))); %tmp.dummy = min, tmp.f_idx(i)=argmin
    end
    c.freqs = tmp.f(tmp.f_idx);
end    

c.lower  = c.freqs(1:c.num_filt);
c.center = c.freqs(2:c.num_filt+1); %40 frequency bins (ends)
c.upper  = c.freqs(3:c.num_filt+2);

%% ignore filters outside of spectrum
[tmp.dummy, tmp.idx] = max(find(c.center <= p.fs/2));
c.num_filt = min(tmp.idx,c.num_filt); %40 frequency bins, minus frequency bins exceeding p.fs/2
...
% rescale amplitude from [-1,1] to [-max wav, max wav] (default is dB max = 96dB = 2^16 wav)
wav = wav * (10^(p.dB_max/20));

mel = zeros(c.num_filt, frames); %40 x #frames
ceps = zeros(p.num_ceps_coeffs-1+p.use_first_coeff, frames); %20 x #frames
if p.visu,
    ffts = zeros(p.fft_size/2+1, frames); %129 x #frames
end
...
mel(mel<1) = 1; %% for dB
%convert amplitude from wav to dB: [0, dB max]
mel=10*log10(mel); %% compute dB -> 40 mel frequency bands
mfcc = DCT*mel; %% compress using DCT -> 20 mfccs

if p.visu, %% intermediate results of processing steps    
    
    %plot 129 frequency bins vs. time frames, indicate intensity (dB) by color
    figure;
    subplot(4,1,1); %% fft
    set(gca,'fontsize',12)
    ffts(ffts<1) = 1; ffts = 10*log10(ffts); imagesc(ffts); set(gca,'ydir','normal','xtick',[])
    title('Power Spectrum (dB)'),xlabel('Time Frame #'),ylabel('FFT Frequency Bin #')
    colorbar
    
    %plot 40 mel frequency bands vs. time frames, indicate intensity (dB) by color
    subplot(4,1,2);
    set(gca,'fontsize',12)
    imagesc(mel); set(gca,'ydir','normal','xtick',[])    
    title('Mel Power Spectrum(dB)'),xlabel('Time Frame #'),ylabel('Mel Frequency Band #')
    colorbar
    
    %plot 20 MFCCs vs. time frames, indicate coefficient value by color
    subplot(4,1,3); 
    set(gca,'fontsize',12)
    imagesc(mfcc); set(gca,'ydir','normal','xtick',[])
    title('MFCC Values'),xlabel('Time Frame #'),ylabel('MFCC Coefficient #')
    colorbar
    
    %plot reconstructed/smoothed 40 mel frequency bands vs. time frames, indicate intensity (dB) by color
    subplot(4,1,4); %% decompressed coefficients
    set(gca,'fontsize',12)
    imagesc(DCT'*mfcc); set(gca,'ydir','normal','xtick',[])    
    title('Reconstructed Smoothed Mel Power Spectrum (dB)'),xlabel('Time Frame #'),ylabel('Mel Frequency Band #')
    colorbar
end